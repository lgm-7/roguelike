...
export const Attack = (player, monster, logs) => {
  if (player.debuf) {
    ...
    if (player.items.dagger === 1) {
      ...
      logs.push(
        chalk.redBright(`단검소지 효과로 몬스터에게 ${aatk}의 추가피해를 입혔습니다. 몬스터의 남은 HP: ${monster.hp}`)
      );
    }
    ...
    if (monster.hp <= 0) {
      ...
      // 몬스터 반격
      handleMonsterAttack(player, monster, logs);
    }
  }
};

const handleMonsterAttack = (player, monster, logs) => {
  const ma = monster.attack(player);
  if (ma === 0) {
  ...
  if (monster instanceof Troll) {
    ...
    logs.push(chalk.redBright(`트롤의 재생력으로 몬스터가 ${msheal}의 HP를 회복 했습니다. 몬스터의 남은 HP: ${monster.hp}`));
  }
  if (monster instanceof Witch) {
    if (getRandom(1, 100) > 85) {
      monster.debuf(player);
      logs.push(chalk.blueBright('플레이어가 마녀의 저주를 받아 다음턴에 아이템 사용을 제외한 행동이 무시됩니다.'));
    }
  }
  if (monster instanceof Salamander) {
    if (getRandom(1, 100) > 60) {
      monster.fire(player);
      logs.push(chalk.blueBright('플레이어가 화상을 입었습니다 3턴동안 몬스터의 공격에 추가로 1의 데미지를 입습니다.'));
    }
  }
};
